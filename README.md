...
| :--- | :--- | :--- |
| **Google Cloud Storage** | `roles/storage.objectAdmin` | Required to stage source code ZIP bundles inside GCS buckets. |
...
> [!TIP]
> **Staging Bucket Auto-Creation:** The deployment tool scans for existing default staging buckets (e.g., `run-sources-[PROJECT]-[REGION]`). If no writable staging bucket exists, it will attempt to automatically provision one on your behalf.

---

## 🚀 Deployment Boilerplate Internals

Every landing page generated is highly secure and self-contained. The microservice boilerplate implements:
- **Dynamic Port Binding:** Automatically binds to port `8080` inside the docker image as expected by Cloud Run.
- **Client-side CORS Middleware:** Enabled to allow seamless static page API requests from any origin.
- **Container Sandboxing:** Uses a lightweight `python:3.11-slim` container base ensuring ultra-fast cold-starts on serverless systems.
- **Rate-Limiter Protection:** Uses an sliding window IP rate limiter preventing credential-stuffing and spam registers:
  ```python
  # Sliding window rate-limiter checking client IP
...
This project is fully supported by your agentic companion **Antigravity**. If you need to modify the agent's system prompt, adapt the frontend CSS generation framework, customize container scaling params, or expand lead analysis features—just ask! Happy Growth Hacking! 🚀
